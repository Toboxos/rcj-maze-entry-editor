<template>
  <div v-if="!authorized" class="w-full h-screen bg-red-500">
  ...
  <div v-if="authorized" class="p-4 text-right">
...
function login() {
  API.axios.post("login", {
    ...
    password: password.value,
      ...
      .then(res => authorized.value = res.status === 200)
      .catch(err => authorized.value = false)
}

function logout() {
  API.axios.get("logout")
